...
function setLocalStorage() {
  if (localStorage.getItem('server') != "") {
    ...
    var data2 = document.getElementById('current_shared_mix_name').value;
    if (data != '' && data2 != '') {

      var name_mix = JSON.parse(localStorage.getItem('name_mix_st') || "[]");
      name_mix.push(data2);
      localStorage.setItem('name_mix_st', JSON.stringify(name_mix));
...
function getLocalStorage() {
  ...
  var storedMixName = JSON.parse(localStorage.getItem("name_mix_st"));
  ...
  if (!storedMix) {
    ...
    for (var i = 0; i < storedMix.length; i++) {
      temp += '<li class="li-mix"> <div class="d-flex justify-content-between"> ' + storedMix[i] + '<div class="d-flex justify-content-end"> ' + '<button id="' + i.toString() + '" onclick="playTune(this)" class="ml-3 mix-btn-st btn  fs-btn-ply sve-btn">Play Mix</button>' +
